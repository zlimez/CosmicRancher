...
namespace engine
{
    ...
    {
        sprite.texture.textureID = graphics::texFromFile(sprite.texture.file, sprite.texture.directory);
        std::string shaderKey = std::string(sprite.shaders.vtxPath) + sprite.shaders.delim + std::string(sprite.shaders.fragPath);
        if (!shaders.count(shaderKey))
            shaders[shaderKey] = graphics::Shader(sprite.shaders.vtxPath, sprite.shaders.fragPath);
